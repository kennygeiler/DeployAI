...
- **⁺²** — `deployment_strategist` / `fde` gained **`admin:read`** during the pilot refresh: the `/admin` pages (MCP activity, Agent Kenny dashboard) are tenant-scoped read-only telemetry, and both roles already hold the stronger `internal:proxy` that serves those pages their data — withholding the page-level action was an inconsistency, not a security boundary.
