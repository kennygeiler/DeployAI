...
- **Scope:** Rust-only `reqwest` (`control_plane_health`, `edge_agent_register_with_control_plane` → `POST /internal/v1/edge-agents/register`, `edge_agent_refresh_kill_switch_from_control_plane` → `GET /internal/v1/edge-agents/by-device`, both with `X-DeployAI-Internal-Key`). No `tauri-plugin-http` exposure to the UI. URL allowlist hardening is still recommended before production.
