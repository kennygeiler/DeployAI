...
| H1 | P0 | M | — | Production backups. **2026-08-11 Railway migration note:** the Fly-era answer (nightly `pg_dump` → Tigris via `fly-backup.yml` + `scripts/fly-backup.sh`) was retired — workflow deleted, scripts archived under `scripts/archive/`. Current posture: Railway **volume backups** (schedule in dashboard) + documented manual `pg_dump`-over-TCP-proxy path (`docs/ops/backup.md`). Still open in spirit: scheduled *offsite* dumps + restore drill for Railway. |
...
| H3 | P0 | M | — | S3-backed artifact stores: email bodies + transcripts currently local-disk stub / `NotImplementedError` for S3 — wire to MinIO locally, an S3-compatible bucket in the cloud (was "Tigris on Fly"; stack now on Railway). Local-disk in a cloud container = data loss on redeploy. |
...
| K6 | P1 | M | S5 | **DONE 2026-08-11** — Guided demo tour: 10-step spotlight overlay on the guest demo (portfolio → Brief → ask Kenny → citations → the refusal trap → Review inbox → Graph → finale) that advances when the visitor actually performs each action. `demo_tour=1` cookie from `/api/auth/demo` mounts `TourProvider` in the strategist layout; steps in `apps/web/src/lib/tour/steps.ts`; skip/resume in sessionStorage. |
