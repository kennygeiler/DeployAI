...
**Current position — 2026-05-23.** Phases 0–4 are delivered (PRs #90–#105) — the team-tracking pivot. **The forward roadmap was re-scoped on 2026-05-22** (see *Direction reset* below) around the product's actual goal — a shared-memory and insight platform that maps a deployment and surfaces cross-team insight. **Phase 5 (the converged deployment-matrix model) is delivered** — increments 5.1–5.5 (PRs #108–#113). **Phase 6.1–6.2c are delivered** (PRs #114–#117) — the full ingest → event → agent → proposal → review → matrix loop runs end to end. **Local testing infrastructure** (repeatable seed of a realistic gov/policy engagement + dev tenant header injection so the loop is browser-testable without SSO) is in flight as [PR #118](https://github.com/kennygeiler/DeployAI/pull/118). **The MVP scope was set on 2026-05-23** — user picked the full per-engagement + cross-engagement insight set (Phase 7), the two paste-based provider harnesses (6.3.1 + 6.3.2), and a proposal-review polish increment. Phase 7.1 (synthesis design record) is the current in-progress increment. **This section is the handoff point** — an agent or developer resuming the work starts here.
...
| 6 | Ingestion + agent extraction — interactions land as events; Cartographer proposes matrix entities | In progress — 6.1, 6.2a, 6.2b, 6.2c done; 6.3 (provider harnesses) next |
| 7 | Synthesis — Oracle / Master Strategist produce insight, suggestions, learnings, opportunities over the matrix | In progress — 7.1 (design) underway; 7.2–7.5 planned |
...
| 6.2c | **Cartographer extractor + chained ingest** — implements the design: `control_plane/agents/matrix_extractor.py`, a `POST /engagements/{id}/extract` endpoint, the BFF chaining extract from `/ingest` best-effort. With this, the loop is *agent → proposal → human review → matrix*. | Merged — [PR #117](https://github.com/kennygeiler/DeployAI/pull/117) |
| 6.3.1 | **Email-paste harness** — paste raw email (RFC 822 or just body + headers), normalize to `IngestInteractionCreate`, no OAuth. Cheapest path to feel real-shaped ingestion ahead of provider OAuth. | Planned |
| 6.3.2 | **Meeting-notes paste harness** — same shape for Otter / Granola / Fireflies exported transcripts. Format detection chooses the right parser. | Planned |
| 6.3.3 | **Gmail OAuth harness** (deferred — post-MVP) — real OAuth, poll inbox, ingest threaded messages. Production-grade. | Deferred |
...
Goal: with the matrix populated by Phase 5 (manual) and Phase 6 (ingest + agent extraction), the **Oracle** and **Master Strategist** agents read the matrix + canonical memory and produce **cross-team insight**, **suggestions**, **learnings**, and **opportunity detection** (where to scale or introduce new offerings). Retrieval-bound and citation-enveloped, per §1 / §11. This is the *synthesis* layer — distinct from Phase 6 *extraction*. **Re-scoped 2026-05-23** into the increment plan below following the MVP scope call (user picked the full per-engagement + cross-engagement insight set, on-demand trigger only, deferring nightly/auto-chained refresh to v2).

| # | Increment | Status |
| --- | --- | --- |
| 7.1 | **Synthesis design record** — finalize insight types (per-engagement + cross-engagement), agent shape (Oracle + Master Strategist as pure functions, same `LLMProvider` injection pattern as Cartographer), trigger model (on-demand refresh button), storage (`matrix_insights` table), dedup_key idempotency. Deliverable: [`synthesis-agents.md`](./synthesis-agents.md). Design-only PR. | In progress |
| 7.2 | **Oracle agent + per-engagement `/insights` CP API** — migration `0024_matrix_insights`, `MatrixInsight` ORM, `control_plane/agents/oracle.py` pure function (deterministic predicates → one LLM call to phrase), per-engagement routes (`GET/POST .../insights`, `.../refresh`, `.../{id}/dismiss`, `.../{id}/resolve`), unit + integration tests. | Planned |
| 7.3 | **Insights surface on engagement detail page** — web BFF + types + `EngagementInsights.client.tsx` above the matrix view; dismiss / resolve / refresh wired. Tests w/ MSW. Closes the per-engagement loop. | Planned |
| 7.4 | **Master Strategist + cross-engagement portfolio insights** — `master_strategist.py` pure function (recurring risk pattern / system concentration / role-coverage gap), per-tenant CP routes, BFF, `PortfolioInsights.client.tsx` on `/engagements`. Closes the cross-team-insight loop. | Planned |
| 7.5 | **Polish — proposal review UX** — dedup grouping (same title across events), bulk accept/reject, filter by kind. Driven by observed gap testing on the seeded Acme County engagement (~200 pending proposals after a single seed run). | Planned |

**Deferred from MVP scope (revisit after user validation):** Phase 6.3.3 (Gmail OAuth), Polish.2 (graph rendering of the matrix view — current view is a list). Both are large investments with no insight payoff; the on-demand paste harness (6.3.1/6.3.2) covers the same ingestion shape for v1 testing.
...
- [`synthesis-agents.md`](./synthesis-agents.md) — Phase 7.1 design record: the Oracle (per-engagement) and Master Strategist (cross-engagement) synthesis agents.
...
| 2026-05-22 | **Phase 6 increment 6.2c** ([PR #117](https://github.com/kennygeiler/DeployAI/pull/117)) — Cartographer extraction agent: `control_plane/agents/matrix_extractor.py` (pure function with fake-LLM unit tests), `get_llm_provider()` factory (anthropic / stub via env), `POST /engagements/{id}/extract?event_id=&force=` route (idempotent by event id), and the web BFF `/ingest` chaining `/extract` best-effort. Catalogs moved to `domain/canonical_memory/matrix.py` (single source of truth). The full agent loop runs end to end. |
