> **Archived 2026-05-27** — covered the Tauri edge agent (Rust/Sparkle/Cargo), `apps/foia-cli` (Go), and other
> retired product lines. The current dev-environment doc lives at the same path (`docs/dev-environment.md`) and
> reflects only the active stack (Node, pnpm, Python/uv, Docker). Preserved for git history.
