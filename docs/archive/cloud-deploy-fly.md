...
> moved from Fly.io to Railway; the current runbook is
> [`docs/ops/cloud-deploy.md`](../ops/cloud-deploy.md). This file is kept as
> history only and its claims were **not re-verified** on archival — the Fly
> apps it describes may no longer exist. Do not follow it for a new deploy.
