> **ARCHIVED 2026-08-11 — superseded by the Railway migration.** The cloud stack
> moved from Fly.io to Railway; the current architecture doc is
> [`docs/ops/cloud-deploy-architecture.md`](../ops/cloud-deploy-architecture.md).
> Kept as history only; **not re-verified** on archival (the Cloudflare Access
> perimeter described here was never wired — OIDC became the auth path).

# Cloud deploy architecture — Fly.io (historical)
