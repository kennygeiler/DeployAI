...
- After commit, the service checks that the new tenant has **no** `canonical_memory_events` rows (explicit `tenant_id` filter so the check is correct even when the app connects with a superuser that bypasses RLS).

## KMS / DEK (AR4)

- `DEPLOYAI_TENANT_DEK_MODE=stub` (default): the service generates 32 random bytes, Base64-encodes them, and uses key id `stub-local`. No AWS calls. Suitable for local and automated tests.
