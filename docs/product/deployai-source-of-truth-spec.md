...
| 5 | Converged deployment-matrix model — structured map on canonical memory | In progress — 5.1 done, 5.2a underway |
...
| 5.2a | **Matrix schema, grain fix & domain models** — migrations `0020` (`matrix_nodes` + `matrix_edges`) and `0021` (nullable `engagement_id` on the canonical-memory tables — the deferred increment 1.3); domain models. | In progress |
...
**Increment 5.2 is split into 5.2a (schema) and 5.2b (API)** — migrations deserve a focused, separately-reviewed PR. The `engagement_log_entries` journal stays live through 5.2–5.3 and is retired in 5.4, after the map view and structured capture replace it.
