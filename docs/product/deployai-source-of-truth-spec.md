...
| 6 | Ingestion + agent extraction — interactions land as events; Cartographer proposes matrix entities | In progress — 6.1, 6.2a done; 6.2b (design) underway, 6.2c (build) next |
...
| 6.2b | **Extraction-agent design record** — finalize trigger, LLM injection, prompt shape, persistence, idempotency, mocking, guardrails. Deliverable: [`matrix-extraction-agent.md`](./matrix-extraction-agent.md). Design-only PR. | In progress |
| 6.2c | **Cartographer extractor + chained ingest** — implements the design: `control_plane/agents/matrix_extractor.py`, a `POST /engagements/{id}/extract` endpoint, the BFF chaining extract from `/ingest` best-effort. With this, the loop is *agent → proposal → human review → matrix*. | Not started |
