...
**Conclusion for planning:** Epic 8 is **not** “complete” in the sense of every **Given/When/Then** in `epics.md`. It **is** ready as a **composable base** for hardening (data wiring, performance tests, a11y journey scripts, nav completeness). **Epic 9** strategist reactive flows are **story-complete** on `main` for scoped stories; **CP-backed durable queues** vs BFF mocks remain a later increment (see [whats-actually-here.md](../../whats-actually-here.md)).
