...
**Date:** 2026-04-26 · **Epic 8 in planning:** [epics.md §Epic 8](../planning-artifacts/epics.md#epic-8-morning-digest-phase-tracking--evening-synthesis-surfaces)
...
| **8.7** | Degraded agents + ingest | `load-strategist-activity.ts` (CP `ingestion-runs`); `StrategistShell` + context; `AgentOutageBanner` when degraded; “Ingesting…” in `ChromeTopBar`. | Real Oracle `agent_error` bus; integration test toggling error on all three surfaces. |
