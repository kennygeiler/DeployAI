...
| `cloud-deploy.yml` | `workflow_run` on **CI** (branch `main`, completed); `workflow_dispatch` | Deploys the 5 Fly.io services in waves (postgres → control-plane → web/mcp-server/embedder). Gated on CI success by the `ci-gate` job (Wave 0 C3 — deploys no longer race CI); path relevance re-checked in-job via `git diff` because `workflow_run` has no native path filter | Pilot ops | active | Ops lane |
