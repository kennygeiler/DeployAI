...
|---|---|---|---|---|---|
| `ci.yml` | PR against `main`; push to `main` | Toolchain guard, **smoke** (`pnpm turbo` build/lint/typecheck/test + contract gate + prettier), job **Control plane (integration)** — `uv run pytest tests/integration/ -m integration` in `services/control-plane` (Docker + Postgres testcontainers), **Ingest (pytest)** (`services/_shared/ingestlib`), path-filtered **MCP server (pytest)** and **LLM provider py (pytest)** (Wave 0 C2 — a path-filter skip counts as pass; push to `main` always runs them). SBOM, Grype CVE, Dependency Review in other jobs. | NFR62 (SBOM), NFR65 (CVE), FR27 contract gate, Story 1.1 deferred AC4 + AC5; integration coverage for control-plane | active | Story 1.2 |
...
| `cloud-deploy.yml` | `workflow_run` on **CI** (branch `main`, completed); `workflow_dispatch` | Deploys the 5 Railway services in waves (postgres → control-plane → web/mcp-server/embedder) via `railway up --service <name>` (requires the `RAILWAY_TOKEN` repo secret). Gated on CI success by the `ci-gate` job (Wave 0 C3 — deploys no longer race CI); path relevance re-checked in-job via `git diff` because `workflow_run` has no native path filter. (2026-08-11: migrated from Fly.io; the nightly `fly-backup.yml` was deleted — production backups are Railway volume backups, see `docs/ops/backup.md`) | Pilot ops | active | Ops lane |
...
| `gate-1-hosted-verification.yml` | `workflow_dispatch` | Optional hosted probes for [phase-0 Gate 1](../../docs/pilot/phase-0-checklist.md): CP `/healthz` JSON + optional web TLS/status — gated by `GATE1_HOSTED_CHECKS_ENABLED` + secrets. See [docs/production/ci-hosted-gate-1.md](../../docs/production/ci-hosted-gate-1.md). | Pilot readiness | optional | Ops lane |
...
- Current source of truth: root `README.md` + `docs/agent-kenny/INDEX.md` (pre-v2 spec archived at `docs/archive/product/deployai-source-of-truth-spec.md`)
