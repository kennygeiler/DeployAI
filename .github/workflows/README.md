...
| `storybook.yml` | PR or push to `main`; `workflow_dispatch` | `pnpm turbo run build-storybook` for `@deployai/web`, uploads `storybook-static` artifact; optional **Chromatic** when `secrets.CHROMATIC_PROJECT_TOKEN` is set (fork-safe) | Epic 7 UX-DR43, design review, optional visual baselines | active; optional Chromatic | Epic 7 |
