...
**Node / pnpm / turbo on your machine:** Use **Node 24.x** as in [`.nvmrc`](./.nvmrc) and the root `engines` field — not e.g. Node 25 — or `pnpm` will error with `ERR_PNPM_UNSUPPORTED_ENGINE`. See [docs/dev-environment.md](./docs/dev-environment.md) for the full install order.
