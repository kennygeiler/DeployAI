...
func VerifyEdgeTranscriptBundleDir(bundleDir string, publicKeyB64 string, skipTSA bool) error {
	...
	if mf.Format != EdgeTranscriptFormat {
		return fmt.Errorf("unsupported format %q (want %s)", mf.Format, EdgeTranscriptFormat)
	}
