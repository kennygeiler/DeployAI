...
import { cpSnoozeGapAsk } from "@/lib/internal/gap-asks-cp";
import { nextResponseFromStrategistCpFetchError } from "@/lib/internal/strategist-bff-cp-error";
import { strategistQueueBffCpMisconfiguredResponse } from "@/lib/internal/strategist-queues-route-guard";

type Ctx = { params: Promise<{ engagementId: string; askId: string }> };

/** Wave 5 GA2 — snooze a gap ask; it resurfaces after `days`. */
export async function POST(request: NextRequest, ctx: Ctx) {
  ...
  try {
    const dismissal = await cpSnoozeGapAsk(tid, engagementId, askId, {
      days,
