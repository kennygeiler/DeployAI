...
import { cpRejectMatrixProposal } from "@/lib/internal/matrix-cp";
...
/**
 * Phase 6 (increment 6.2a) — reject a matrix proposal. Sets status to
 * "rejected" with server-derived actor attribution; no matrix change.
 */
export async function POST(_request: NextRequest, ctx: Ctx) {
  ...
  try {
    const proposal = await cpRejectMatrixProposal(tid, engagementId, proposalId, {
      actor_id: actorId,
    });
