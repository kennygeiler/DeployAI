...
import { cpResolveTenantInsight } from "@/lib/internal/insights-cp";
...
/**
 * Phase 7 (increment 7.4) — resolve a portfolio (tenant-scoped) insight.
 */
export async function POST(_request: NextRequest, ctx: Ctx) {
  ...
  try {
    const insight = await cpResolveTenantInsight(tid, insightId, { actor_id: actorId });
