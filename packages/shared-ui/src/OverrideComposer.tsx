...
export function OverrideComposer({
  ...
  const [touched, setTouched] = React.useState({ what: false, why: false, evidence: false });
  ...
  const runSubmit = async () => {
    setTouched({ what: true, why: true, evidence: true });
    setFormError(null);
    if (what.trim().length === 0 || why.trim().length === 0 || selected.size === 0) {
      return;
    }
    setSubmitting(true);
    setStatus("idle");
  ...
  return (
    ...
    >
      ...
      {status === "success" ? (
        <div
          className="md:col-span-2 rounded-md border border-border bg-paper-200 px-3 py-2 text-sm text-ink-800"
          role="status"
        >
          Override request recorded. Propagation is queued for the surfaces below (preview).
        </div>
      ) : null}
