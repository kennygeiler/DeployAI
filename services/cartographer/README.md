...
- **Story 6-2 (FR20):** `src/cartographer/extract.py` — `extract_stub` (deterministic), `extraction_bundle_to_persist_dict`, `bundle_fingerprint`. `src/cartographer/llm_extract.py` — `extract_map_reduce_llm` (Anthropic by default; inject `completer(chunk)->str` in tests). Control plane: `services/cartographer_extraction_persist.py` appends `cartographer.extraction` via `ingestion_dedup_key`. Integration test: `services/control-plane/tests/integration/test_cartographer_extraction_persist.py` + `tests/fixtures/cartographer/email_thread.json`. **NFR48 hook:** `python -m cartographer.benchmark` (stub wall time). **Logs:** `DEPLOYAI_CARTOGRAPHER_TRIAGE_LOG_JSON=1` omits message bodies from triage logs.
