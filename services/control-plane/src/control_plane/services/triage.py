"""Mission-relevance triage (salvaged from the retired cartographer service) — no LLM; deterministic score ∈ [0, 1]."""
...
# Operational rate = irate/rate() over this counter; counter name uses the
# Prometheus _total suffix (OpenMetrics style). Label cardinality: avoid
# unbounded custom phase strings in production; prefer registered phase ids.
TRIAGE_OUTCOMES: Counter = Counter(
