...
The extraction pipeline may only consume events that pass :func:`validate_extraction_queue_event`.
"""
...
class PerMessageExtractionError(ExtractionUnitError):
    """A per-message (or *message-suffixed) event must not enter the extraction queue."""
...
def validate_extraction_queue_event(*, event_type: str) -> ExtractionUnitKind:
    ...
    Callers publishing to the extraction queue (extraction pipeline, Epic 6) must
